...
void huffman::saveDecodedFile()
{
    ...
    // reading count at the end of the file which is number if bits appended to make to  final value 8-bit
    inFile.seekg(-1, ios::end);
    char count0;
    inFile.read(&count0, 1);
    // Ignoring the ,eta data (huffman tree) (1+17*size) and reading remaining file
...
void huffman::getTree()
{
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        // converting decimal characters into binary equivalent to obtain code
        string hCodeStr = "";
        for (int i = 0; i < 16; i++)
        {
